...
import { MapPin, Phone, Star, Mail, ArrowRight, Calendar, Clock } from "lucide-react";
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-5xl mx-auto pt-32 pb-12 px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className={`flex-1 space-y-12 ${locale === 'ar' ? 'order-1 lg:order-1' : 'order-1'}`}>
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-1 space-y-4">
                <div>
                  <h1 className="text-2xl font-bold text-[#001A6E]">{doctor.name}</h1>
                  <p className="text-gray-500 font-medium">{t("specialties.doctors", locale)} {doctor.specialty}</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-4 rounded-2xl">
                    <p className="text-xs text-gray-400 mb-1">{t("booking.sessionFee", locale)}</p>
                    <p className="font-bold text-[#001A6E]">{doctor.price} {locale === "ar" ? "ج.م" : "EGP"}</p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-2xl">
                    <p className="text-xs text-gray-400 mb-1">{t("booking.city", locale)}</p>
            ...
            {/* About Doctor */}
            <section>
              <h2 className="text-xl font-bold text-[#001A6E] mb-4">{t("booking.aboutDoctor", locale)}</h2>
              <p className="text-gray-500 leading-relaxed text-sm">
                {doctor.about || (locale === "ar" ? "استشاري متخصص بخبرة واسعة في مجاله، يحرص على تقديم أفضل رعاية طبية للمرضى باستخدام أحدث التقنيات الطبية العالمية." : "Specialized consultant with extensive experience in his field, keen to provide the best medical care to patients using the latest global medical technologies.")}
              </p>
            </section>
          ...
          <div className={`lg:w-80 flex-shrink-0 ${locale === 'ar' ? 'order-2 lg:order-2' : 'order-2'}`}>
            <div className="bg-white border border-gray-100 rounded-[32px] p-6 shadow-sm sticky top-32">
              <h2 className="text-xl font-bold text-[#001A6E] mb-6">{t("booking.clinicInfo", locale)}</h2>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="font-bold text-gray-800 mb-3">{t("booking.workingHours", locale)}</h3>
        ...
        <div className="mt-20 py-12 border-t border-b border-gray-100">
          <div className="text-center mb-12">
            <h2 className="text-2xl font-bold text-[#001A6E] mb-4">{t("booking.patientReviews", locale)}</h2>
            <div className="flex flex-col items-center">
              ...
              <p className="font-bold text-lg text-gray-800">{t("booking.overallRating", locale)}</p>
              <p className="text-gray-400 text-sm">{t("booking.fromVisitors", locale)}</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
            {clinic.reviews.slice(0, 4).map((review) => (
              <div key={review.id} className="bg-gradient-to-br from-blue-50 to-white border border-blue-100 p-6 rounded-3xl shadow-sm hover:shadow-md transition-shadow">
                <div className="flex gap-0.5 mb-3">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className="w-4 h-4 text-yellow-400 fill-yellow-400" />
                  ))}
                </div>
                <p className="font-bold text-sm text-gray-800 mb-2">{t("booking.overallRating", locale)}</p>
                <p className="text-gray-600 text-xs font-medium mb-1">{review.name}</p>
