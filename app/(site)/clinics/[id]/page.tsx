    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-6xl mx-auto pt-32 pb-12 px-4">
        ...
        <div className="flex flex-col md:flex-row gap-12 mb-20">
          ...
          <div className={`md:w-1/2 relative ${locale === 'ar' ? 'order-1 md:order-2' : 'order-1'}`}>
          ...
          <div className={`flex-1 ${locale === 'ar' ? 'order-2 md:order-1' : 'order-2'}`}>
            <h2 className="text-2xl font-bold mb-2 text-[#001A6E]">{t("clinics.aboutClinic", locale)}</h2>
            <p className="text-gray-400 mb-8">{clinic.name}</p>
            
            <h3 className="text-xl font-bold mb-6">{t("clinics.medicalSpecialties", locale)}</h3>
            ...
            <div className="space-y-4 mb-8">
              <div className="flex items-center text-gray-400">
                <MapPin className={`w-5 h-5 ${locale === 'ar' ? 'ml-3' : 'mr-3'}`} />
                <span>{clinic.city}</span>
              </div>
              <div className="flex items-center text-gray-400">
                <Phone className={`w-5 h-5 ${locale === 'ar' ? 'ml-3' : 'mr-3'}`} />
                <span dir="ltr">{clinic.phone}</span>
              </div>
              <div className="flex items-center text-gray-400">
                <Clock className={`w-5 h-5 ${locale === 'ar' ? 'ml-3' : 'mr-3'}`} />
            ...
            <p className="text-gray-300 text-xs">{t("clinics.fromVisitors", locale).replace("{count}", "245")}</p>
          </div>
        </div>

        {/* Doctors Section */}
        <div className="mb-20">
          <h2 className="text-2xl font-bold mb-10 text-[#001A6E]">{t("clinics.doctors", locale)}</h2>
          ...
          <div className="flex flex-wrap gap-4 mb-12 justify-center">
            <div className="relative min-w-[200px]">
              ...
              <ChevronDown className={`w-4 h-4 absolute ${locale === 'ar' ? 'left-4' : 'right-4'} top-1/2 -translate-y-1/2 pointer-events-none text-gray-400`} />
            </div>
        ...
        {/* Location Section */}
        <div className="mb-20">
          <h2 className="text-2xl font-bold mb-10 text-[#001A6E]">{t("clinics.clinicLocation", locale)}</h2>
        ...
        {/* Reviews Section */}
        <div className="mb-20">
          <h2 className="text-2xl font-bold mb-12 text-[#001A6E]">{t("clinics.patientReviews", locale)}</h2>
          ...
          <div className="text-center mb-12">
            ...
            <p className="font-bold text-lg text-gray-800">{t("clinics.overallRating", locale)}</p>
            <p className="text-gray-400 text-sm">{t("clinics.fromVisitors", locale).replace("{count}", "200")}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
            {clinic.reviews.slice(0, 4).map((review) => (
              <div key={review.id} className="bg-gradient-to-br from-blue-50 to-white border border-blue-100 p-6 rounded-3xl shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="font-bold text-sm text-gray-800 mb-2">{t("clinics.overallRating", locale)}</p>
