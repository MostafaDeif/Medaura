  ...
  return (
    <section className="rounded-[30px] border border-[#d8e3ff] bg-white px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#4b69c7]">{t("sales.label", locale)}</p>
          <h2 className="mt-2 text-2xl font-extrabold text-[#001a6e] sm:text-3xl">{t("sales.title", locale)}</h2>
        </div>
        <Link
          href="/auth/register"
        ...
        {offers.map((offer) => (
          ...
          >
            <div className="relative h-48 w-full">
              <Image src={`/images/${offer.image ?? 'sale1.png'}`} alt={offer.title} fill className="object-cover" />
            </div>
            <div className="space-y-3 p-5">
              <h3 className="text-lg font-extrabold text-[#0f1a4f]">{offer.title}</h3>
              <p className="text-sm leading-6 text-[#5a6d9d]">{offer.description}</p>
